...
router.get('/:id/market-price', async (req, res) => {
  try {
    const itemId = parseInt(req.params.id);
    const marketPrices = await marketPriceService.getMarketPricesByItemId(itemId);
    if (!marketPrices.length) {
      return res.status(404).json({ error: 'No market price data found for this item' });
    }
